 ...
 * @subpackage classes
 */
class MantisFilterMultiInt extends MantisFilter {
	...
	public function processGPC() {
		if ( is_array( gpc_get( $this->field, $this->filter_value ) ) ) { 
			$this->filter_value = gpc_get_int_array( $this->field, $this->filter_value );
		} else {
			$f_value = gpc_get_int( $this->field, $this->filter_value );
	...
	public function display() {
		...
		if( $this->has_any && $this->isAny() ) {
			$t_display_arr[] = array( 'value'=>META_FILTER_ANY, 'label'=>string_display( lang_get( 'any' ) ) );
		} else {
			...
			foreach( $this->filter_value as $t_current ) {
				...
				if( $this->has_none && $this->isNone() ) {
					$t_display_arr[] = array( 'value'=>META_FILTER_NONE, 'label'=>string_display( lang_get('none') ) ) ;
				} else {
					$t_display_arr[] = array( 'value'=>$t_current, 'label'=>string_display( $t_current ) ) ;
				}
			}
		}
		return $t_display_arr;
	}
