 ...
 * @subpackage classes
 */
class MantisFilterOS extends MantisFilterMultiString {
	...
	public function options() {
		log_event( LOG_FILTERING, 'OS = ' . var_export( $this->filter_value, true ) );
 		$t_os_array = profile_get_field_all_for_user( 'os' );
		foreach( $t_os_array as $t_os ) {
			$t_os = string_attribute( $t_os );
			$t_selected = in_array( $t_os, $this->filter_value ) ? true : false;
			$t_options[] = array( 'value'=>string_attribute( $t_os ), 'label'=>string_display( $t_os ), 'selected'=>$t_selected );
		}
		return $t_options;
	}
}
