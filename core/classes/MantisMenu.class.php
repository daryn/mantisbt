...
class MantisMenu {
	...
	/**
	 *	Boolean whether or not to sort the submenus alphabetically.
	 */
	...
	/**
	 *	Add a link to the links list for this menu.  If the p_link param is a
	 *	cooked link, parse out the displayed string and
	 *	add both the link and the string as an array. If the param is an array
	 *	just add it to the links array.  Otherwise, the param should be a
	 *	key to an element in the Mantis available links array.
	 *	
	 *	@param mixed $p_link p_link may be a cooked link, 
	 *	an array of data in the format array( 'id'=>value,'url'=>value,'label'=>value )
	 *	for building a link or a string key	to the available Mantis links.
	 */
	 ...
	 *	@param string $p_link link identifier
	 *	@return bool
	 */
	...
	/**
	 *	Print the menu and in submenus.
	 */
	public function __toString() {
		$t_is_child = false;
		if( config_get( 'nested_menus' ) == ON ) {
			if( $this->alpha_sort == ON ) {
				usort( $this->links, 'self::cmp' );
			}
		...
		foreach( $this->links AS $t_key=>$t_link ) {
			...
			# child menu here
			if( config_get( 'nested_menus' ) == ON && array_key_exists( $t_key, $this->submenus ) ) {
	...
	public static function getAccountMenu() {
		if( !array_key_exists( 'account', self::$menus ) ) {
			...
			$t_account->addLink( 'columns_preferences' );
	...
	public static function getAvailableLinks() {
		if( count( self::$mantis_links ) == 0 ) {
			...
			if( auth_is_user_authenticated() ) {
				...
				$t_links['columns_preferences'] = 'account_manage_columns_page.php';
