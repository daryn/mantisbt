...
function db_connect( $p_dsn, $p_hostname = null, $p_username = null, $p_password = null, $p_database_name = null, $p_db_options = null ) {
    ...
    # @todo move db factory call to an App scope class
    $t_db_factory = new DB\DatabaseFactory();
	$g_db = $t_db_factory->create($t_db_type);
...
function db_result( $p_result, $p_index1 = 0, $p_index2 = 0 ) {
	...
	//var_dump($p_index1);
	//var_dump($p_index2);
	//var_dump($p_result->fetchColumn($p_index2));
	//die;
	return $p_result->fetchColumn($p_index2);
...
function db_index_exists( $p_table_name, $p_index_name ) {
	...
	$t_indexes = $g_db->getIndexes( $p_table_name );
